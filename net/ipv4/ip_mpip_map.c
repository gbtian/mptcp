...
int update_path_info(unsigned char session_id, unsigned int len)
{
	...
	if (count == 4)
	{
		list_for_each_entry(sp, &sorted_list, list)
		{
			...
			if(next_sp)
			{
				...
				printk("%d, %d: %s, %s, %d\n", sp->path_info->path_id, next_sp->path_info->path_id, __FILE__, __FUNCTION__, __LINE__);
	...
	list_for_each_entry(path_info, &pi_head, list)
	{
		...
		if (path_info->delay > max_delay || max_delay == -1)
		{
		...
		if (path_info->min_delay > max_min_delay || max_min_delay == -1)
		{
		...
		if (path_info->queuing_delay > max_queuing_delay || max_queuing_delay == -1)
		{
